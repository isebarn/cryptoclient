...
const Core = new function () {
  ...
  const isMobile = !!navigator.userAgent.toLowerCase().match(/ipod|ipad|iphone|android/gi)
  ...
  // Types of organisms
  const ORGANISM_ENEMY = 'enemy'
  const ORGANISM_ENERGY = 'energy'
  ...
  // Mouse properties
  let mouseX = (window.innerWidth + world.width) * 0.5
  let mouseY = (window.innerHeight + world.height) * 0.5
  ...
  let fs = 0 // Frame score
  let cs = 0 // Collision score

  // The world's velocity
  const velocity = { x: -1.3, y: 1 }

  // Performance (FPS) tracking
  let fps = 0
  let fpsMin = 1000
  let fpsMax = 0
  let timeLastSecond = new Date().getTime()
  let frames = 0
  ...
  function renderBackground () {
    const gradient = contextBackground.createRadialGradient(world.width * 0.5, world.height * 0.5, 0, world.width * 0.5, world.height * 0.5, 500)
  ...
  function windowResizeHandler () {
    ...
    const cvx = (window.innerWidth - world.width) * 0.5
    const cvy = (window.innerHeight - world.height) * 0.5
  ...
  function animate () {
    ...
    let scoreFactor = 0.01 + (Math.max(Math.min(fps, FRAMERATE), 0) / FRAMERATE * 0.99)
    ...
    for (i = 0; i < organisms.length; i++) {
      ...
      const angle = Math.atan2(p.position.y - player.position.y, p.position.x - player.position.x)
